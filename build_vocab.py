...
def build_vocab(json_path, threshold):
    ...
    with open(json_path, 'r') as f:
        ann = json.load(f)    
    counter = Counter()
    img_list = list(ann.keys())
    #pdb.set_trace()
    for i, id in enumerate(img_list):
        img_id = i
        text_id = np.random.randint(0,4,1)[0]
        img_name = img_list[img_id]
        caption = ann[img_name]['caption'][text_id]
        tokens = nltk.tokenize.word_tokenize(caption.lower())
        counter.update(tokens)

        if (i+1) % 1000 == 0:
            print("[{}/{}] Tokenized the captions.".format(i+1, len(img_list)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--caption_path', type=str,
                        default='../../Data/YunDisk/data_bcidr/train_annotation.json',
                        help='path for train annotation file')
    parser.add_argument('--vocab_path', type=str, default='../../Data/YunDisk/data_bcidr/vocab.pkl',
